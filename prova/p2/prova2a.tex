...
Observe que os valores da variáveis $j$ e $k$ na fun\c{c}ão {\it main} correspondem aos dois últimos dígitos de {\bf seu} RGA, respectivamente. Assim, se o seu RGA é 201121902014, então $j=1$ e $k=4$.
\begin{verbatim}
#include <stdio.h>
...
\vspace{\baselineskip} {\bf 2.} Escreva uma fun\c{c}ão chamada {\tt união} que recebe dois conjuntos de mesmo tamanho {\tt n}, armazenados nos vetores {\tt A} e {\tt B}.
Esta fun\c{c}ão deve calcular a união entre {\tt A} e {\tt B} e armazenar o resultado no vetor {\tt C}. Desta maneira, {\tt C} deverá conter os elementos que estiverem em {\tt A} e em {\tt B}, desconsiderando as repeti\c{c}ões. A fun\c{c}ão deverá devolver o tamanho do vetor {\tt C}.

{\Large\begin{tabular}{|l|} \hline
\verb+int uniao(int A[], int B[], int C[], int n) {+ \\ \hline % 1
...
\vspace{\baselineskip} {\bf 3.} Escreva uma fun\c{c}ão que recebe um vetor de números inteiros {\it v} e o seu tamanho {\it n} e verifica se {\it v} está ordenado.
Se {\it v} estiver ordenado, sua fun\c{c}ão deve devolver  $1$ e, $0$, caso contrário.
 

{\Large\begin{tabular}{|l|} \hline
\verb+int esta_ordenado(int v[], int n) {+\\ \hline % 1
\hspace*{\textwidth} \mbox{}\\ \hline %2
\mbox{}\\ \hline %3
\mbox{}\\ \hline %4
\mbox{}\\ \hline %5
